...
PNG createSpotlight(PNG image, int centerX, int centerY) {
  for (unsigned x = 0; x < image.width(); x++) {  //since width() is set to be insigned int, x must be unsigned rather than just int
    for (unsigned y = 0; y < image.height(); y++) {
      HSLAPixel & pixel = image.getPixel(x, y);
      ...
      //   diff_y = centerY - y;
      // }
      double diff_x = fabs((int) x - centerX);
      double diff_y = fabs((int) y - centerY);
...
PNG watermark(PNG firstImage, PNG secondImage) {
  for (unsigned x = 0; x < firstImage.width() && x < secondImage.width(); x++) {
    for (unsigned y = 0; y < firstImage.height() && y < secondImage.height(); y++) {
      ...
      if (secondPixel.l == 1) {
        if ((firstPixel.l + 0.2) <= 1) {
          firstPixel.l = firstPixel.l + 0.2;
        } else {
          firstPixel.l = firstPixel.l;
        }
      } else {
        firstPixel.l = firstPixel.l;
      }
