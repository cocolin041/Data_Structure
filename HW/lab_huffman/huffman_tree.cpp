                            ...
                            queue<TreeNode*>& mergeQueue)
{
    ...
    if (singleQueue.size() > 0 && mergeQueue.size() > 0) {
        TreeNode *smallest_single = singleQueue.front();
        TreeNode *smallest_merge = mergeQueue.front();
        if (smallest_single->freq < smallest_merge->freq || smallest_single->freq == smallest_merge->freq) {
            singleQueue.pop();
            TreeNode *smallest = smallest_single;
            return smallest;
        } else {
            mergeQueue.pop();
            TreeNode *smallest = smallest_merge;
            return smallest;
        }
    } else if (singleQueue.size() + mergeQueue.size() > 0) {
        if (singleQueue.size() == 0 && mergeQueue.size() > 0) {
            TreeNode *smallest_merge = mergeQueue.front();
            mergeQueue.pop();
            return smallest_merge;
        }
        if (singleQueue.size() > 0 && mergeQueue.size() == 0) {
            TreeNode *smallest_single = singleQueue.front();
            singleQueue.pop();
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    for (int i = 0; i < frequencies.size(); i++) {
        TreeNode *curr = new TreeNode(frequencies[i]);
        singleQueue.push(curr);
    }
    while (singleQueue.size() == 0 && mergeQueue.size() == 1) {
        TreeNode *child_1 = removeSmallest(singleQueue, mergeQueue);
        TreeNode *child_2 = removeSmallest(singleQueue, mergeQueue);
        TreeNode *parent = new TreeNode(child_1->freq.getFrequency() + child_2->freq.getFrequency());
        parent->left = child_1;
        parent->right = child_1;
        mergeQueue.push(parent);
    }
    this->root_ = mergeQueue.front();
    mergeQueue.pop();
