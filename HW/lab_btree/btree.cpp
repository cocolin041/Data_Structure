...
void BTree<K, V>::split_child(BTreeNode* parent, size_t child_idx)
{
    ...
    if (child->children.size() != 0) {
        new_right->children = child->children;
        new_right->children.assign(new_right->children.begin() + mid_child_idx, new_right->children.end());
        child->children.assign(child->children.begin(), mid_child_itr);
    }

    new_right->elements = child->elements;
    new_right->elements.assign(new_right->elements.begin() + mid_elem_idx + 1, new_right->elements.end());
    child->elements.assign(child->elements.begin(), mid_elem_itr);
...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
    ...
    if (subroot->elements.size() == 0) {
        ...
        //if this node is not a leaf, insert on the child[index]
        if (!subroot->is_leaf) {
            BTreeNode* child_to_insert = subroot->children[first_larger_idx];
            insert(child_to_insert, pair);
        } else {
        //if this node is a leaf, insert to this node element
            if (first_larger_idx < subroot->elements.size()) {  //if the index is not exceed element size, insert between elements
                first_larger_idx = insertion_idx(subroot->elements, pair);
                subroot->elements.insert(subroot->elements.begin() + first_larger_idx, pair);
            } else { //if the index is more than element size, push at the back
                subroot->elements.push_back(pair);
            }
        }
    }
    ...
    // split on this child
    if (!subroot->is_leaf) {
        for (unsigned i = 0; i < subroot->children.size(); i++) {
