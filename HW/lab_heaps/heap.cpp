...
size_t heap<T, Compare>::parent(size_t currentIdx) const
{
    ...
    size_t size = _elems.size();
    double parent = (double)currentIdx / 2;
    size_t parentIdx = std::floor(parent);
    if (parentIdx > 0) {
        return parentIdx;
    }
    return 0;
...
T heap<T, Compare>::pop()
{
    ...
    if ((int)size > 0) {
        ...
        std::swap(_elems[root()], _elems[size - 1]);

        // remove the last element from _elems
        T minElem = _elems[size - 1];
        _elems.pop_back();

        // mantain the property of heap
        heapifyDown(root());
