...
class KDTree
{
  ...
  private:
     ...
     */
    void DestroyRecursive(KDTreeNode*& node);
    KDTreeNode* CopyRecursive(const KDTreeNode* node);

    void swap(Point<Dim>& x, Point<Dim>& y);
    int partition(vector<Point<Dim>>& arr, int l, int r, int dim);
    Point<Dim> kthSmallest(vector<Point<Dim>>& arr, int l, int r, int k, int dim);
    KDTreeNode* KDTree_helper(const vector<Point<Dim>> newPoints, vector<Point<Dim>>& copyOfPoints, int left, int right, int dim);
    
    double distance(const Point<Dim>& first, const Point<Dim>& second) const;
    KDTreeNode* findNearestNeighbor_helper(const Point<Dim>& query, KDTreeNode* subroot, int dim) const;
};
