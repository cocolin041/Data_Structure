...
void DisjointSets::setunion(int a, int b) {
  ...
  //store the new size to be the sum of two sets' sizes
  int newSize = sets[a] + sets[b];

  //compare two sets' size, let the more elements' set to be the new root
  if ( sets[a] <= sets[b] ) {
    sets[b] = a;
    sets[a] = newSize;
  } else {
...
int DisjointSets::size(int elem) {
  int size = 0;
  //if elem is not the root index, find the root index firstly
  while (sets[elem] >= 0) {
    elem = sets[elem];
  }
  //since the element of root is negative size, add a negative sign to it
  size = -sets[elem];
  return size;
};