...
void Room::clear()
{
    if (letters != NULL) {                              //memory error here: letters is an array, must be deleted with a []
        delete [] letters;
    }
}
...
void Room::copy(const Room& other)
{
    ...
    // letters = other.letters;                         //memory error here: 1) letter is a pointer, should do deep copy with new 2) when assign value into letters, since it's an array, we should use for loop
