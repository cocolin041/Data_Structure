...
void Allocator::loadRooms(const string& file)
{
    ...
    rooms = new Room[roomCount];            //memory error here: uninitialized error: initialize roomCount one line before
...
// Add destructor to free those variables that's stored on heap (which has been 'new')
Allocator::~Allocator(){
    if (alpha != NULL) {
        delete [] alpha;
    }
