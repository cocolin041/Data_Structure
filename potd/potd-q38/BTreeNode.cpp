...
bool underflows(unsigned numElem, unsigned order) {
    // TODO: Your code here!
    double half = (double)order / 2;
    if (numElem < ceil(half) - 1) {
        return true;
    } else {
        return false;
    }
...
bool BTreeNode::rotateRight(unsigned idx, unsigned order) {
     ...
     */
    if (idx <= 0)   return false; //idx <= 0 means this leaf is the first element of the root
    ...
    int lastLeftChild = prev->elements_[prev->elements_.size() - 1];
    int &firstParentKey = this->elements_[0];
    int lastParentKey = this->elements_[this->elements_.size() - 1];
    
    std::vector <int> &rightChild = children_[idx]->elements_;
    std::vector <int> &leftChild = prev->elements_;
    std::vector <int> &parent = this->elements_;

    rightChild.insert(rightChild.begin(), lastParentKey);
    leftChild.erase(leftChild.end() - 1);

    if (parent.size() > 1) {
        for (unsigned i = 0; i < parent.size() - 1; i++) {
            parent[i + 1] = parent[i];
        }
    }
